...
fun sumRateVkPay(sumMonth: Int = 0, amount: Int): Int {
...
fun checkTransferAllCard(
    ...
    val dayLimit = 150_000_00 //дневной лимит перевода в день по всем картам, кроме VK pay
...
fun sumRate(typeCard: String = "MasterCard" , amountDay: Int = 0, sumMonth: Int =0, amount: Int) {
    var result: Int = -1 //вычисленная комиссия за перевод
    when (typeCard) {
        "MasterCard", "Maestro" -> if (checkTransferAllCard(amountDay, sumMonth, amount))
            result = sumRateMasterMaestro(sumMonth, amount)

        "Visa", "Мир" -> if (checkTransferAllCard(amountDay, sumMonth, amount))
...
fun main() {
    val sumMonth = 78000_00 //общая сумма переводов за текущий месяц без учета комиссий
    val amountDay = 1000_00 //сумма переводов за текущий день
    val typeCard = "Visa" //тип карты отправителя
    val amount = 500_00 //сумма превода
    ...
    sumRate(typeCard, amountDay, sumMonth, amount)
}
